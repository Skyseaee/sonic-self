...
// true for vaild while false for invaild
#define is_legal_char(ch) (ch == '+' || ch == '-' || ch == '<' || ch == '>' || ch == '[' || ch == ']' || ch == '.' || ch == ',')


// read file and write into a char*
char* read_file(FILE *f) {
    char buf[MAX_BUF];
    char* str = (char*)malloc(sizeof(buf) + 1);
    size_t cap = sizeof(buf), size = 0, s = 0;
    while((s = fread(buf, 1, sizeof(buf), f)) > 0) {
        if(size+s > cap) {
            cap *= 2;
            str = (char*)realloc(str, cap+1);
        }
